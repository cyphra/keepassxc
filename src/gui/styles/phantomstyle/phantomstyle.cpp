    ...
    namespace
    {
        ...
        Q_NEVER_INLINE void PhSwatch::loadFromQPalette(const QPalette& pal)
        {
            ...
            for (int i = 0; i < Num_ShadowSteps; ++i) {
                scrollbarShadowColors[i] = gutterGrad.sample((qreal)i / (qreal)Num_ShadowSteps);
            }
        }
...
namespace Phantom
{
    namespace
    {
        ...
        Q_ALWAYS_INLINE QRect rectTranslatedTowardEdge(QRect rect, Qt::Edge edge, int delta)
        {
            switch (edge) {
            case Qt::LeftEdge:
                                                   ...
                                                   Swatchy fill)
        {
            ...
            if (radius > 0.5) {
                ...
                p->setBrush(swatch.brush(fill));
                QRectF rf((qreal)rect.x() + 0.5,
                          (qreal)rect.y() + 0.5,
                          (qreal)rect.width() - 1.0,
                          (qreal)rect.height() - 1.0);
...
void PhantomStyle::drawItemText(QPainter* painter,
                                const QRect& rect,
                                int alignment,
                                const QPalette& pal,
                                bool enabled,
                                 ...
                                 const QWidget* widget) const
{
    ...
    case PE_FrameGroupBox: {
        ...
        if (isFlat) {
            ...
            Ph::paintBorderedRoundRect(painter, frame, Ph::GroupBox_Rounding, swatch, S_window_divider, S_none);
        }
        break;
    }
    ...
    case PE_IndicatorArrowLeft: {
        ...
        if (arrow == Qt::DownArrow && !(bool)qstyleoption_cast<const QStyleOptionToolButton*>(option) && widget) {
                               ...
                               const QWidget* widget) const
{
    ...
    case CE_TabBarTabShape: {
        ...
        QRect drawRect = Ph::expandRect(shapeClipRect, innerEdge, 3 + 2 * (int)rounding + 1);
        if (!onlyOne && !lastTab) {
        ...
        Ph::paintBorderedRoundRect(painter, drawRect, rounding, swatch, S_window_outline, thisFillColor);
                                      ...
                                      const QWidget* widget) const
{
    ...
    case CC_ComboBox: {
        ...
        if (comboBox->subControls & SC_ComboBoxArrow) {
            int margin =
                (int)((qreal)qMin(downArrowRect.width(), downArrowRect.height()) * Ph::ComboBox_ArrowMarginRatio);
...
int PhantomStyle::pixelMetric(PixelMetric metric, const QStyleOption* option, const QWidget* widget) const
{
    int val = -1;
    switch (metric) {
    ...
    case PM_MenuBarHMargin:
        ...
        return (int)((qreal)widget->fontMetrics().height() * Phantom::MenuBar_HorizontalPaddingFontRatio);
    ...
    case PM_TabBarTabHSpace:
        ...
        if (!option)
            break;
        return (int)((qreal)option->fontMetrics.height() * Phantom::TabBar_HPaddingFontRatio)
               + (int)Phantom::dpiScaled(4);
    case PM_TabBarTabVSpace:
        if (!option)
            break;
        return (int)((qreal)option->fontMetrics.height() * Phantom::TabBar_VPaddingFontRatio)
                                     ...
                                     const QWidget* widget) const
{
    ...
    case CT_GroupBox: {
        ...
        if (opt->subControls & (SC_GroupBoxCheckBox | SC_GroupBoxLabel)) {
            ...
            yadd += (int)((qreal)fontHeight * Phantom::GroupBox_LabelBottomMarginFontRatio);
        }
    ...
    case CT_PushButton: {
        ...
        if (!pbopt || pbopt->text.isEmpty())
            break;
        int hpad = (int)((qreal)pbopt->fontMetrics.height() * Phantom::PushButton_HorizontalPaddingFontHeightRatio);
...
void PhantomStyle::unpolish(QApplication* app)
{
...
QRect PhantomStyle::itemPixmapRect(const QRect& r, int flags, const QPixmap& pixmap) const
{
    return QCommonStyle::itemPixmapRect(r, flags, pixmap);
}
void PhantomStyle::drawItemPixmap(QPainter* painter, const QRect& rect, int alignment, const QPixmap& pixmap) const
{
    QCommonStyle::drawItemPixmap(painter, rect, alignment, pixmap);
}
QStyle::SubControl PhantomStyle::hitTestComplexControl(ComplexControl cc,
...
QPixmap PhantomStyle::generatedIconPixmap(QIcon::Mode iconMode, const QPixmap& pixmap, const QStyleOption* opt) const
{
    return QCommonStyle::generatedIconPixmap(iconMode, pixmap, opt);
}

int PhantomStyle::styleHint(StyleHint hint,
                            const QStyleOption* option,
...
QRect PhantomStyle::subElementRect(SubElement sr, const QStyleOption* opt, const QWidget* w) const
{
    switch (sr) {
